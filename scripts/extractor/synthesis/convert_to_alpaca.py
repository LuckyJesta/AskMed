...
from scripts.extractor.pipeline.fact_validator import validate_training_projection
...
def build_alpaca_row(row: dict) -> dict[str, str]:
    ...
    runtime_output = row.get("parsed_output") or {"facts": []}
    parsed_output = clean_extraction_for_training(runtime_output)
    errors = validate_training_projection(runtime_output, parsed_output)
    if errors:
        raise ValueError("projection_error: " + "; ".join(errors))
    input_text = format_alpaca_input(source)
    leaked = [key for key in ("meddg_weak_labels", "dialogue_id", "turn_id") if key in input_text]
    if leaked:
        raise ValueError("projection_error: Alpaca input leaked internal keys: " + ",".join(leaked))
    return {
