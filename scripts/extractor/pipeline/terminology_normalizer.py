...
class TerminologyNormalizer:
    ...
    def __init__(self, db_path: str | Path | None = None) -> None:
        ...
        self._lookup_cache: dict[tuple[str, str, str], TerminologyMatch | None] = {}
    ...
    def ensure_database_readable(self) -> None:
        if not self.enabled:
            return
        with closing(sqlite3.connect(self.db_path)) as conn:
            conn.execute("select 1 from terms limit 1").fetchone()

    def verify_coded_fact(self, fact: dict[str, Any]) -> bool:
        code = fact.get("standard_code")
        terminology = fact.get("terminology")
        if not code and not terminology:
            return True
        if not code or not terminology or not self.enabled:
            return False
        semantic_type = str(fact.get("type") or "")
        expected = SUPPORTED_TYPES.get(semantic_type)
        if expected is not None and terminology != expected:
            return False
        with closing(sqlite3.connect(self.db_path)) as conn:
            row = conn.execute(
                """
                select 1 from terms
                where semantic_type = ? and terminology = ? and code = ?
                limit 1
                """,
                (semantic_type, str(terminology), str(code)),
            ).fetchone()
    ...
    def match_fact(self, fact: dict[str, Any]) -> TerminologyMatch | None:
        ...
        if expected_terminology is None and not self.enabled:
            return None
        ...
        for raw_name in names:
            ...
            if self.enabled:
                match = (
                    self._lookup_exact(semantic_type, expected_terminology, key)
                    if expected_terminology is not None
                    else self._lookup_exact_any(semantic_type, key)
                )
        ...
        with closing(sqlite3.connect(self.db_path)) as conn:
            ...
            if expected_terminology is None:
                rows = conn.execute(
                    ...
                    from terms where semantic_type = ?
                    """,
                    (semantic_type,),
                ).fetchall()
            else:
                rows = conn.execute(
                    """
                    select terminology, semantic_type, code, preferred_name, alias
                    from terms where semantic_type = ? and terminology = ?
                    """,
    ...
    def _lookup_exact(self, semantic_type: str, terminology: str, normalized_key: str) -> TerminologyMatch | None:
        cache_key = (semantic_type, terminology, normalized_key)
        ...
        self._lookup_cache[cache_key] = match
        return match

    def _lookup_exact_any(self, semantic_type: str, normalized_key: str) -> TerminologyMatch | None:
        cache_key = (semantic_type, "*", normalized_key)
        if cache_key in self._lookup_cache:
            return self._lookup_cache[cache_key]
        with closing(sqlite3.connect(self.db_path)) as conn:
            conn.row_factory = sqlite3.Row
            rows = conn.execute(
                ...
                select terminology, semantic_type, code, preferred_name, alias
                from terms
                where semantic_type = ? and normalized_key = ?
                order by preferred_name = alias desc, id asc
                """,
                (semantic_type, normalized_key),
            ).fetchall()
        identities = {(row["terminology"], row["code"]) for row in rows}
        match = None
        if len(identities) == 1 and rows:
            row = rows[0]
            match = TerminologyMatch(
                semantic_type=row["semantic_type"],
                terminology=row["terminology"],
