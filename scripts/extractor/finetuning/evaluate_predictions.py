...
FORBIDDEN_ATTRIBUTE_KEYS = REQUIRED_FACT_FIELDS - {"attribute"}
...
def validate_schema(value: dict[str, Any]) -> bool:
    ...
    for fact in facts:
        ...
        if not isinstance(fact.get("attribute"), dict):
            return False
        if set(fact["attribute"]) & FORBIDDEN_ATTRIBUTE_KEYS:
            return False
...
def attribute_keys_overlap(left: Any, right: Any) -> bool:
    if not isinstance(left, dict):
        left = {}
    if not isinstance(right, dict):
        right = {}
...
def extract_patient_utterance(sample_input: str) -> str:
    """Extract the patient turn from the stable Alpaca input projection."""
    match = re.search(
        r"(?:^|\n)患者当前发言：(.*?)(?:\n当前问诊状态：|$)",
        sample_input or "",
        flags=re.DOTALL,
...
def main() -> None:
    ...
    for prediction_row, sample in zip(predictions, dataset):
        ...
        utterance_text = extract_patient_utterance(input_text)
        try:
            processed = fact_pipeline.process(
                predicted,
                patient_utterance=utterance_text,
                state=empty_patient_state("evaluation"),
                turn_id=None,
            )
            predicted = processed.training_extraction
        ...
        for predicted_idx, gold_idx in core_pairs:
            ...
            for field in ("normalized_name", "evidence"):
            ...
            field_totals["attribute_key_overlap"] += 1
            if attribute_keys_overlap(predicted_attr, gold_attr):
                field_correct["attribute_key_overlap"] += 1
        ...
        for fact in predicted_list:
            ...
            if isinstance(evidence, str) and evidence:
                ...
                if utterance_text and evidence in utterance_text:
    ...
    report = {
        ...
        "prediction_postprocessing": {
            ...
            "failures": postprocessing_failures,
        },
