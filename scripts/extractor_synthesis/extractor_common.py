...
10. 输出前逐条自检：如果某个fact的evidence不是患者当前发言的连续子串，必须删除该fact，不要改写或借用上下文证据。
...
7. 输出顶层格式为 {"facts":[...]}；每个fact包含name, normalized_name, type, status, subject, time, body_part, attribute, evidence, standard_code, terminology。
8. type只能是symptom/disease/medicine/examination/attribute/history/lifestyle/other；subject只能是patient/family/other/unknown。
9. attribute必须是JSON对象；time和body_part无信息时为null；standard_code和terminology固定为null。
10. 短回答如“三天了”“五六次”“没有”只有能确定目标时才抽取；否定短回答必须输出absent且evidence使用患者否定词原文，不能复制医生问题；无法判断目标时输出 {"facts":[]}。"""
