...
def build_user_prompt(row: dict) -> str:
    return (
        ...
        "【输出要求】\n"
        "1. 只抽取患者当前发言表达的事实。\n"
        "2. evidence必须来自患者当前发言的连续原文片段。\n"
        "3. 如果当前发言只是寒暄、确认、感谢或告别且没有新医学信息，输出 {\"facts\":[]}。\n"
        "4. 如果当前发言是短回答，结合上一轮医生问题和当前问诊状态判断目标；无法判断目标时不要硬编。\n"
        "5. 如果当前发言中症状本体和属性同时出现，把属性合并进该症状fact，不要重复输出attribute fact。\n"
        "6. 请只输出紧凑JSON，不要换行、缩进或多余空格。"
    )
