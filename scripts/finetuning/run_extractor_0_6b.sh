...
OUTPUT_ROOT="${PROJECT_ROOT}/outputs/extractor_qwen3_0_6b"
TRAIN_OUTPUT="${OUTPUT_ROOT}/train"
TEST_OUTPUT="${OUTPUT_ROOT}/test"
SMOKE_OUTPUT="${OUTPUT_ROOT}/smoke"
TRAIN_CONFIG="${CONFIG_DIR}/extractor_qwen3_0_6b_lora.yaml"
PREDICT_CONFIG="${CONFIG_DIR}/extractor_qwen3_0_6b_predict.yaml"

export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"
export TOKENIZERS_PARALLELISM="${TOKENIZERS_PARALLELISM:-false}"

mkdir -p "${PROJECT_ROOT}/models" "${TRAIN_OUTPUT}" "${TEST_OUTPUT}" "${SMOKE_OUTPUT}"
cd "${PROJECT_ROOT}"
...
download_model() {
  stage 2 "Prepare Qwen3-0.6B with ModelScope"
...
train_model() {
  stage 3 "Qwen3-0.6B LoRA training and validation"
  require_model
...
smoke_test() {
  stage 3 "Qwen3-0.6B 20-step smoke training"
  require_model
...
test_model() {
  stage 4 "Qwen3-0.6B test prediction and structured metrics"
  require_model
...
usage() {
  cat <<EOF
Usage: bash scripts/finetuning/run_extractor_0_6b.sh <command>

Commands:
  check      Validate LLaMA-Factory, CUDA, BF16 and datasets
  download   Download ${MODEL_ID} into models/
  smoke      Run a 200-sample, 20-step end-to-end training check
  train      Train and validate the full LoRA adapter
...
Environment variables:
  ...
  RESUME_FROM_CHECKPOINT     Optional 0.6B checkpoint path
